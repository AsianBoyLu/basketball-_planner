...
export default function BallerPro() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("ballerCareerV_FINAL_FIX");
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem("ballerCareerV_FINAL_FIX", JSON.stringify({ shots: shotSessions, notes, events, seasons }));
    ...
    <div style={s.container}>
      <header style={{ textAlign: 'center', marginBottom: '15px' }}>
        ...
        <div style={{display:'flex', justifyContent:'center', gap:'10px', marginTop: '5px'}}>
           <select style={{ background: '#1e293b', color: 'white', border: 'none', borderRadius:'5px' }} value={activeSeason} onChange={e => setActiveSeason(e.target.value)}>
             {seasons.map(sn => <option key={sn} value={sn}>{sn}</option>)}
           </select>
           <button onClick={addSeason} style={{background:'#334155', color:'white', border:'none', borderRadius:'5px', padding:'2px 8px', fontSize:'0.7rem'}}>+ Season</button>
        </div>
      </header>
      ...
      {page === 'career' && (
        <div style={{...s.card, display:'grid', gridTemplateColumns:'1fr 1fr', gap:'20px', textAlign:'center'}}>
          <div><div style={{fontSize:'0.6rem', color:'#94a3b8'}}>PTS</div><div style={{fontSize:'1.8rem', fontWeight:'900', color:'#ea580c'}}>{careerTotals.pts}</div></div>
          <div><div style={{fontSize:'0.6rem', color:'#94a3b8'}}>REB</div><div style={{fontSize:'1.8rem', fontWeight:'900', color:'#ea580c'}}>{careerTotals.reb}</div></div>
          <div><div style={{fontSize:'0.6rem', color:'#94a3b8'}}>AST</div><div style={{fontSize:'1.8rem', fontWeight:'900', color:'#ea580c'}}>{careerTotals.ast}</div></div>
          <div><div style={{fontSize:'0.6rem', color:'#94a3b8'}}>TOV</div><div style={{fontSize:'1.8rem', fontWeight:'900', color:'#ea580c'}}>{careerTotals.tov}</div></div>
        </div>
      )}
        ...
        <div>
          <div style={s.card}>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', gap: '15px', borderTop: '1px solid #334155', paddingTop: '10px' }}>
              <div style={{textAlign:'center'}}><div style={{fontSize:'1rem', fontWeight:'bold', color:'#ea580c'}}>{latest.box.pts}</div><div style={{fontSize:'0.5rem', color:'#94a3b8'}}>PTS</div></div>
              <div style={{textAlign:'center'}}><div style={{fontSize:'1rem', fontWeight:'bold', color:'#ea580c'}}>{latest.box.reb}</div><div style={{fontSize:'0.5rem', color:'#94a3b8'}}>REB</div></div>
              <div style={{textAlign:'center'}}><div style={{fontSize:'1rem', fontWeight:'bold', color:'#ea580c'}}>{latest.box.ast}</div><div style={{fontSize:'0.5rem', color:'#94a3b8'}}>AST</div></div>
              <div style={{textAlign:'center'}}><div style={{fontSize:'1rem', fontWeight:'bold', color:'#ea580c'}}>{latest.box.tov}</div><div style={{fontSize:'0.5rem', color:'#94a3b8'}}>TOV</div></div>
            </div>
          </div>
          <div style={s.card}>
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '10px' }}>
              <div>
                <div style={{display:'flex', justifyContent:'space-between', fontSize:'0.7rem'}}>3PT <input style={s.input} value={m3} onChange={e=>setM3(e.target.value)}/>/<input style={s.input} value={a3} onChange={e=>setA3(e.target.value)}/></div>
                <div style={{display:'flex', justifyContent:'space-between', fontSize:'0.7rem', marginTop:5}}>MID <input style={s.input} value={mMid} onChange={e=>setMMid(e.target.value)}/>/<input style={s.input} value={aMid} onChange={e=>setAMid(e.target.value)}/></div>
                <div style={{display:'flex', justifyContent:'space-between', fontSize:'0.7rem', marginTop:5}}>LAY <input style={s.input} value={mLay} onChange={e=>setMLay(e.target.value)}/>/<input style={s.input} value={aLay} onChange={e=>setALay(e.target.value)}/></div>
                <div style={{display:'flex', justifyContent:'space-between', fontSize:'0.7rem', marginTop:5}}>FT <input style={s.input} value={mFt} onChange={e=>setMFt(e.target.value)}/>/<input style={s.input} value={aFt} onChange={e=>setAFt(e.target.value)}/></div>
              </div>
              <div style={{borderLeft:'1px solid #334155', paddingLeft:10}}>
                <div style={{fontSize:'0.7rem'}}>PTS <input style={{...s.input, width:40}} value={pts} onChange={e=>setPts(e.target.value)}/></div>
                <div style={{fontSize:'0.7rem', marginTop:5}}>REB <input style={{...s.input, width:40}} value={reb} onChange={e=>setReb(e.target.value)}/></div>
                <div style={{fontSize:'0.7rem', marginTop:5}}>AST <input style={{...s.input, width:40}} value={ast} onChange={e=>setAst(e.target.value)}/></div>
                <div style={{fontSize:'0.7rem', marginTop:5}}>TOV <input style={{...s.input, width:40}} value={tov} onChange={e=>setTov(e.target.value)}/></div>
              </div>
            </div>
            <button onClick={addSession} style={{ width: '100%', padding: '10px', backgroundColor: '#ea580c', border: 'none', color: 'white', borderRadius: '10px', fontWeight: 'bold', marginTop: '10px' }}>SAVE SESSION</button>
          </div>
        </div>
      )}
        ...
        <div>
          <div style={s.card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
              <button onClick={() => setCurrentMonth(new Date(currentMonth.setMonth(currentMonth.getMonth()-1)))} style={{background:'none', border:'none', color:'white'}}>‹</button>
          ...
          <div style={s.card}>
            <input style={{width:'100%', padding:'10px', borderRadius:'8px', backgroundColor:'#334155', border:'none', color:'white', boxSizing:'border-box'}} placeholder="Plan..." value={planTitle} onChange={e=>setPlanTitle(e.target.value)} />
            <button onClick={() => { if(planTitle){ setEvents({...events, [selectedDate.toDateString()]: [...(events[selectedDate.toDateString()]||[]), {title:planTitle}]}); setPlanTitle(""); } }} style={{width:'100%', marginTop:5, padding:8, backgroundColor:'#ea580c', border:'none', color:'white', borderRadius:8, fontWeight:'bold'}}>ADD</button>
          </div>
        </div>
      )}

      {page === 'notes' && (
        <textarea style={{ width: '100%', height: '65vh', borderRadius: '20px', padding: '15px', backgroundColor: '#1e293b', border: '1px solid #334155', color: 'white', outline: 'none', boxSizing:'border-box' }} placeholder="Notes..." value={notes} onChange={e => setNotes(e.target.value)} />
      )}
    </div>
  );
}
